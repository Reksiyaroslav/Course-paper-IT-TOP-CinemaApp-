...
from app.model.model_actor import (
    ActorResponse,
    ActorCreateRequest,
    ActorUpdateRequest,
    ActorListResponse,
...
class ActorControlle(Controller):
    ...
    async def create_actor(
        self, data: ActorCreateRequest, async_session: AsyncSession
    ) -> ActorResponse:
    ...
    async def get_actor_name(
        self, actor_fistname_or_lastname_or_patronymic: str, async_session: AsyncSession
    ) -> list[ActorResponse]:
        actor_service = get_service(ActorService, ActorRepository, async_session)
        actor = await actor_service.get_fistname_lastname_pat(
            actor_fistname_or_lastname_or_patronymic
        )
        if not actor:
    ...
    async def update_user(
        self, actor_id: UUID, data: ActorUpdateRequest, async_session: AsyncSession
    ) -> ActorResponse:
        actor_service = get_service(ActorService, ActorRepository, async_session)
    ...
    async def delete_actor(
        self, actor_id: UUID, async_session: AsyncSession
    ) -> Dict[str, str]:
        actor_service = get_service(ActorService, ActorRepository, async_session)
