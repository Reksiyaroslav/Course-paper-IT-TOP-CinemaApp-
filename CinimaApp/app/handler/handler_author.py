...
from app.model.model_author import (
    AuthorCreateRequest,
    AuthorResponse,
    AuthorlListResponse,
    AuthorUpdateRequest,
...
class AuthorControlle(Controller):
    ...
    async def create_author(
        self, data: AuthorCreateRequest, async_session: AsyncSession
    ) -> AuthorResponse:
    ...
    async def get_author_id(
        self, author_id: UUID, async_session: AsyncSession
    ) -> AuthorResponse:
        author_service = get_service(AuthorService, AuthorRepository, async_session)
    ...
    async def get_author_fistname_lastname_pat(
        self, author_fistname_lastname_pat: str, async_session: AsyncSession
    ) -> AuthorResponse:
        author_service = get_service(AuthorService, AuthorRepository, async_session)
        author = await author_service.get_fistname_lastname_pat(
            author_fistname_lastname_pat
        )
        if not author:
            raise HTTPException("Not Author", status_code=404)
        return AuthorResponse.from_orm(author)
    ...
    async def update_user(
        self, author_id: UUID, data: AuthorUpdateRequest, async_session: AsyncSession
    ) -> AuthorResponse:
        author_service = get_service(AuthorService, AuthorRepository, async_session)
