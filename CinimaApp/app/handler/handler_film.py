...
class FilmControlle(Controller):
    ...
    @post()
    async def create_films(self,data:FilmCreateRequest,async_session:AsyncSession)->FilmResponse:
    ...
    async def add_film_actorlist(self,film_id:UUID,data:AddActorFilsmResponse,async_session:AsyncSession)->FilmResponse:
        film_service   = get_service(FilmService,FilmRepository,async_session) 
        film = await film_service.add_actors_film_model(data.dict(),film_id)
        return FilmResponse.from_orm(film)
    
    @post("/id/{film_id:uuid}/authors/")
    async def add_film_autorlist(self,film_id:UUID,data:AddAuthorFilsmResponse,async_session:AsyncSession)->FilmResponse:
        film_service   = get_service(FilmService,FilmRepository,async_session) 
    ...
    @get("id/{film_id:uuid}")
    async def get_film_id(self,film_id:UUID,async_session:AsyncSession)->FilmResponse:
        film_service   = get_service(FilmService,FilmRepository,async_session) 
        film = await film_service.get_model(film_id)
        return FilmResponse.from_orm(film)
    
    @get("titel/{film_titel:str}")
    async def get_filim_titel(self,film_titel:str,async_session:AsyncSession)->FilmlListResponse:
        film_service   = get_service(FilmService,FilmRepository,async_session) 
    ...
    async def update_film(self,film_id:UUID, data:FilmUpdateRequest ,async_session:AsyncSession)->FilmResponse:
        film_service   = get_service(FilmService,FilmRepository,async_session) 
