...
class RatingFilmService(Base_Service):
    ...
    async def create_model(self, data, name_title_value=None):
        old_rating = await self.repo.get_user_film_rating(data["film_id"],data["user_id"])
        if old_rating:
            updated_rating = await self.add_update_film_and_user(data["film_id"], data["user_id"], data["rating"])
    ...
    async def add_update_film_and_user(self,film_id:UUID,user_id:UUID,rating):
        ratingfilm=  await self.repo.add_upadate_rating_user_and_film(film_id,user_id,rating)
