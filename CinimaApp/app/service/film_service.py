...
from app.list.list_searhc import list_serach_date,list_serach_name_title,list_serach_rating
    ...
    async def create_model(self, data, name_title_value=None):
        if await validate_is_data_range(data[list_serach_date[0]],"film"):
            if await validet_star_rating(data,list_serach_rating[1]):
                if not await is_name_title(model=self.repo.model,session=self.repo.session,name_filed=list_serach_name_title[3],
