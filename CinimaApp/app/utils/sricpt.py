...
import random
from app.db.model_db.model_db import Film,User,Actor,Author,user_film,author_ciema,film_actor
from app.db.engine import provide_async_session
from sqlalchemy import select,func
import bcrypt
faker = Faker()
async def seend_database():
    async for session in provide_async_session():
        actors =[]
        users = []
        authors = []
        films =[]
        #Созданрие актера 
        for _ in range(1,50):
            actor = Actor(fistname= faker.first_name(),
        ...
        #Создание автора 
        for _ in range(0,30):
            author =Author(fistname= faker.first_name(),
                          lastname = faker.last_name(),
                          patronymic =faker.first_name(),
        ...
        for _ in range(1,4):
            ...
            hath.decode("utf-8")
            user = User(password =  hath.decode("utf-8"),
                        email = faker.email(),
        ...
        for _ in range(1,100):
            film = Film(title = faker.catch_phrase(),
    ...
    print(f"База заполнена! Актеры: {actors_count}, Фильмы: {films_count}, Пользователи: {users_count}")
