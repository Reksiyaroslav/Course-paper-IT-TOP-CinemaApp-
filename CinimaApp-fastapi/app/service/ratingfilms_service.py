...
from app.repositories.ratingfilm_repositore import RatingFilmRepository,RatingFilm as rating_model
...
class RatingFilmService(Base_Service):
    ...
    async def create_ratingfilm(self, data: dict, user_id: UUID, film_id: UUID)->dict |rating_model:
        if not await not_create_rating(session=self.session,model=rating_model,user_id=user_id,film_id=film_id):
    ...
    async def update_rating_user_id_and_film_id(self, data: dict, user_id: UUID,film_id:UUID)->rating_model:
        return await self.rating_repo.update_rating_user_id(user_id=user_id, data=data,film_id=film_id)
