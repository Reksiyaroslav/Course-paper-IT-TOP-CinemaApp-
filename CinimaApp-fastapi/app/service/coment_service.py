...
class ComentService(Base_Service):
    ...
    async def update_comet_like_unlike(self,user_id:UUID,coment_id,type_rec:str):
        if type_rec  not in (Type_Rec.Like.value , Type_Rec.UnLike.value):
            raise HTTPException(
                detail="Вы не можете передавть сюда только unlike like больше не чего ", status_code=400
            )
        ...
        if old_reco is None:
            await self.reco_repo.create_reco(user_id=user_id,coment_id=coment_id,type_rec=type_rec)
        elif old_type ==type_rec:
            if old_reco:
                await self.reco_repo.delete_reco(user_id=user_id,coment_id=coment_id)
        else:
            await self.reco_repo.update_reco(user_id=user_id,coment_id=coment_id,type_rec=type_rec)
        return await self.coment_repo.update_type_recon(coment_id=coment_id,type_recon=type_rec,relult_recon=old_type)
