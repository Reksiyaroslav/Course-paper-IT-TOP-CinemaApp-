...
class RecoRepository():
    ...
    async def create_reco(self,user_id:UUID ,type_rec:str,coment_id:UUID):
        reco:Recone=  Recone(repit_user_id=user_id,coment_recone =coment_id,type_rect=type_rec)
    ...
    async def get_reco_user_id_coment_id(self,user_id,coment_id):
        smt = select(Recone).where(Recone.repit_user_id ==user_id and Recone.coment_recone==coment_id)
    ...
    async def update_reco(self,user_id:UUID ,type_rec:str,coment_id:UUID):
        smt = select(Recone).where(Recone.repit_user_id ==user_id , Recone.coment_recone==coment_id)
        relult = await self.session.execute(smt)
    ...
    async def delete_reco(self,user_id,coment_id):
        smt = delete(Recone).where(Recone.repit_user_id ==user_id , Recone.coment_recone==coment_id)
