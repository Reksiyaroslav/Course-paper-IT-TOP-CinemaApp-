...
from app.db.model.model_db import Film, RatingFilm, Coment,Actor,Author,film_actor,author_ciema
...
class FilmRepository(ModelRepository):
    ...
    async def get_film_relations(self,film_id:UUID):
        stmt =  select(Film).where(Film.id == film_id).options(selectinload(Film.actors),selectinload(Film.authors))
