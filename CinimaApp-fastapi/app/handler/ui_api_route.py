...
from app.utils.enums import Type_model
from app.handler.film_api_handler import get_block_films
...
async def view_item(request: Request, item_id:UUID,env_type_mode:Type_model,
              film_service: FilmService = Depends(get_film_service),
              actor_service: ActorService = Depends(get_actor_service)
              ,user_service:UserService = Depends(get_user_service)) -> dict:
    service= { Type_model.actor:  actor_service.get_actor_by_id,
              Type_model.film:  film_service.get_film_by_id,
              Type_model.user:  user_service.get_user_by_id
    }
...
async def main_pages(request:Request,films= Depends( get_block_films)):
...
async def main_pages_actor(request:Request,actor_server:ActorService= Depends(get_actor_service)):
    actors = await actor_server.get_actor_list()
    return teamlates.TemplateResponse(name="main.html",context={"request":request,"actors":actors})
