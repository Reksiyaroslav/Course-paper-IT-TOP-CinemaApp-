...
class UserControlle(Controller):
    ...
    async def create_user(
        self, async_session: AsyncSession, data: UserCreateRequest
    ) -> UserResponse:
    ...
    async def add_likefilm(
        self, user_id: UUID, likefilm_id: UUID, async_session: AsyncSession
    ) -> UserRepository:
        user_service = get_service(
            UserService, UserRepository, async_session=async_session
        )
    ...
    async def get_user_name(
        self, user_name: str, async_session: AsyncSession
    ) -> UserResponse:
        user_service = get_service(
            UserService, UserRepository, async_session=async_session
        )
    ...
    async def get_password_username(
        self, user_name: str, password: str, async_session: AsyncSession
    ) -> UserResponse:
        user_service = get_service(
            UserService, UserRepository, async_session=async_session
        )
