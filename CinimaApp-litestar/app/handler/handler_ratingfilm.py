...
from app.model.model_ratingfilms import (
    RatingFilmResponse,
...
class RatingFilmControlle(Controller):
    ...
    async def create_ratingfilm(
        self, data: RatingFilmCreateRequest, async_session: AsyncSession
    ) -> RatingFilmResponse:
        ratingfilm_service = get_service(
    ...
    async def add_update_rating_film_and_user(
        self, film_id: UUID, user_id: UUID, rating: int, async_session: AsyncSession
    ) -> RatingFilmResponse:
        ratingfilm_service = get_service(
            RatingFilmService, RatingFilmRepository, async_session
        )
        ratingfilm = await ratingfilm_service.add_update_film_and_user(
            film_id, user_id, rating
        )
        return RatingFilmResponse.from_orm(ratingfilm)

    @get()
    async def get_list_ratingfilm(
    ...
    async def get_rating_id(
        self, rating_film_id: UUID, async_session: AsyncSession
    ) -> RatingFilmResponse:
        ratingfilm_service = get_service(
            RatingFilmService, RatingFilmRepository, async_session
        )
